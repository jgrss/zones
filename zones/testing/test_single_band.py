...
def test_01():
    ...
    assert int(df.loc[0, 'sum']) == feat01.sum()
    assert int(df.loc[1, 'sum']) == feat02.sum()
    assert int(df.loc[2, 'sum']) == feat03.sum()
    assert round(df.loc[0, 'mean'], 2) == round(feat01.mean(), 2)
    assert round(df.loc[1, 'mean'], 2) == round(feat02.mean(), 2)
    assert round(df.loc[2, 'mean'], 2) == round(feat03.mean(), 2)
